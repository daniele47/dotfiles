...
function __cleanup_prompt__() {
    ...
    ###############################################
    local container=""
    [[ -f /run/.toolboxenv ]] && container="${red}[container] "
    ###############################################
    ...
    if [[ -d "$GITDIR/.git" ]]; then
        computations="$(git rev-list --left-right --count '@{u}...HEAD' 2>/dev/null || echo; git status --porcelain 2>/dev/null)"
        if [[ "$computations" == *$'\n'* ]]; then
            _status="${computations#*$'\n'}"
            diff="${computations%%$'\n'*}"
        else
            _status=""
            diff="$computations"
    ...
    *) symbol="${red}❯ " ;;
    ...
    ###############################################
    PS1="${wipe}${container}${workdir}${gitbranch}${gitstate}${gitstatus}${symbol}${wipe}"
